...
// Updates:
//   Revised dacWrite() in ontimer26() ISR to use inline code. This prevents crash during EEPROM writes.
//   Fixed FillBuffer26() so that buffer size can be successfully changed.
...
//   Fixed XT_Wav26_Class and XT_Sequence26_Class sub-volume controls.
//
 ...
 *  Also, variables used by onTimer26 must be regular variables and cannot be
 ...
 *  To avoid re-entrancy problems, the ISR onTimer26() must change only the "play" index and the
 *  mainline code must change only the "fill" index.   Otherwise we can get corruption if
 *  onTimer26() does a pull while FillBuffer26() is putting data in.
 *  Each can _look_ at the other's variables, but both cannot _change_ the same variable.
 */

volatile int32_t NextPlayPos26=0;             // position in buffer of next byte to play
volatile uint8_t *Buffer26;               // The buffer to store the data that will be sent to the
volatile uint16_t Buffer26Size;             // Actual size if buffer used (default can be overridden in function call)
volatile uint8_t DacPin26;                            // pin to send DAC data to, presumably one of the DAC pins!
volatile uint8_t LastDacValue26 = 0x7f;           // Last value sent to DAC, if next is same we don't write to DAC again
XT_PlayListItem26_Class *FirstPlayListItem26=nullptr;       // first play list item to play in linked list. TEB, Oct-02-2019.
XT_PlayListItem26_Class *LastPlayListItem26=nullptr;        // last play list item to play in linked list. TEB, Oct-02-2019.

XT_Instrument26_Class DEFAULT_INSTRUMENT26;

int Buffer26Used = 0;                   // NB : Not working yet. Amount of bytes sent out to play since last buffer fill, this value
                            // can help you determine an optimum buffer size if you don't want to
                            // stick with the default set in BUFFER_SIZE_DEFAULT


// interrupt stuff
hw_timer_t * timer26 = nullptr;                           // TEB, Oct-10-2019
portMUX_TYPE timer26Mux = portMUX_INITIALIZER_UNLOCKED;
...
int SineValues26[256];       // an array to store our values for sine

void InitSineValues26()
{
  ...
  for(int MyAngle=0;MyAngle<256;MyAngle++) {
    ...
    SineValues26[MyAngle]=(sin(RadAngle)*127)+128;    // get the sine of this angle and 'shift' up so
...
uint8_t SetVolume26(uint8_t Value,uint8_t Volume)
{
...
void IRAM_ATTR onTimer26()
{
  // Sound playing code, plays whatever's in the buffer continuously. Big change from previous versions
  if(LastDacValue26!=Buffer26[NextPlayPos26])   // Send value to DAC only if changed since last value else no need
  {
    // value to DAC has changed, send to actual hardware, else we just leave setting as is as it's not changed
    LastDacValue26=Buffer26[NextPlayPos26];

//    dacWrite(DacPin26,LastDacValue26);      // Don't use, will cause reboot during E2Prom writes. See emulation code below.

        // Start of dacWrite() emulation ...
        CLEAR_PERI_REG_MASK(SENS_SAR_DAC_CTRL1_REG, SENS_SW_TONE_EN);  //Disable Tone
        if(DacPin26==25) {
//          pinMode(DacPin26, ANALOG);  // Don't enable, will cause reboot during E2Prom writes. Pre-Set by dacWrite() in XT_DAC_Audio26_Class init.
            CLEAR_PERI_REG_MASK(SENS_SAR_DAC_CTRL2_REG, SENS_DAC_CW_EN1_M);
            SET_PERI_REG_BITS(RTC_IO_PAD_DAC1_REG, RTC_IO_PDAC1_DAC, LastDacValue26, RTC_IO_PDAC1_DAC_S);
            SET_PERI_REG_MASK(RTC_IO_PAD_DAC1_REG, RTC_IO_PDAC1_XPD_DAC | RTC_IO_PDAC1_DAC_XPD_FORCE);
        }
        else if(DacPin26==26) {
//          pinMode(DacPin26, ANALOG);   // Don't enable, will cause reboot during E2Prom writes. Pre-Set by dacWrite() in XT_DAC_Audio26_Class init.
            CLEAR_PERI_REG_MASK(SENS_SAR_DAC_CTRL2_REG, SENS_DAC_CW_EN2_M);
            SET_PERI_REG_BITS(RTC_IO_PAD_DAC2_REG, RTC_IO_PDAC2_DAC, LastDacValue26, RTC_IO_PDAC2_DAC_S);
            SET_PERI_REG_MASK(RTC_IO_PAD_DAC2_REG, RTC_IO_PDAC2_XPD_DAC | RTC_IO_PDAC2_DAC_XPD_FORCE);
        }
        // End of dacWrite() emulation.

  }
  Buffer26[NextPlayPos26]=0x7f;         // Set this buffer byte to silence, TEB Sep-23-2019.
  NextPlayPos26++;                // Move play pos to next byte in buffer
  if(NextPlayPos26 >= Buffer26Size)       // If gone past end of buffer,
    NextPlayPos26=0;              // set back to beginning
}


int XT_DAC_Audio26_Class::Buffer26Usage()
{
   return Buffer26Used;
}

void XT_DAC_Audio26_Class::FillBuffer26()
{
  ...
  XT_PlayListItem26_Class *PlayItem,*NextPlayItem;
  int16_t ByteToPlay,Buffer26ByteToMixWith;             // Need 16 bit signed so we can have a range of
                                  // -255 to +256
//  uint8_t NextByte,Buffer26Byte;                  // raw unsigned values of naext byte to plat and mix with
  ...
  PlayItem=FirstPlayListItem26;

  while(PlayItem!=nullptr)
  {
    if(PlayItem->NewSound)                    // A new unplayed sound, set initial fill buffer position
    {
      PlayItem->NextFillPos=NextPlayPos26+1;          // to one in front of actual playpos
      PlayItem->NewSound=false;               // No longer a new sound now.
      LogAvail=false;
    }

    avail = Buffer26Size- (PlayItem->NextFillPos - NextPlayPos26);  // Work out how much buffer we can fill
    if (avail >=Buffer26Size)                   // This indicates the next fill pos was behind play pos,
      avail -= Buffer26Size;                  //  bring into correct range
    if(LogAvail)                        // calc how much of the play buffer was used since the
    {                             // Last call to FillBuffer26
      Buffer26Used=NextPlayPos26-LastPlayPos;
      if(Buffer26Used<0)
        Buffer26Used=Buffer26Used+Buffer26Size;
    }
    while((PlayItem->Playing) && (avail > 0))         // while space in buffer and item still active.  TEB, Sep-29-2019.
    {
      ...
      Buffer26ByteToMixWith=Buffer26[PlayItem->NextFillPos]-127;  // -127 to make it's a wave that swings above and below 0

      ByteToPlay=Buffer26ByteToMixWith+ByteToPlay;        // Mix them together and store back in ByteToPlay
      if(ByteToPlay>128)
      ...
      Buffer26[PlayItem->NextFillPos]=uint8_t(ByteToPlay+127);  // Store back in buffer adjusting so back in 0-255 range

      // Move the fill position for this sound item to next pos
      PlayItem->NextFillPos++;                // move to next buffer position
      if(PlayItem->NextFillPos>=Buffer26Size)         // Set to start of buffer if end reached
...
XT_PlayListItem26_Class::XT_PlayListItem26_Class()
{
  RepeatForever=false;
  Repeat=0;
  RepeatIdx=0;
...
uint8_t XT_PlayListItem26_Class::NextByte()
{
  return 0;
}
void XT_PlayListItem26_Class::Init()
{
}


XT_DAC_Audio26_Class::XT_DAC_Audio26_Class(uint8_t TheDacPin26, uint8_t TimerNo):XT_DAC_Audio26_Class(TheDacPin26,TimerNo,BUFFER_SIZE_DEFAULT)
{
  // calls other constructor as shown in function definition
}


XT_DAC_Audio26_Class::XT_DAC_Audio26_Class(uint8_t TheDacPin26, uint8_t TimerNo,uint16_t PassedBuffer26Size)
{
  ...
  // that we will call our onTimer26 function 50,000 times a second

  // reserve some memory for the buffer
  Buffer26=new uint8_t[PassedBuffer26Size];
  Buffer26Size=PassedBuffer26Size;          // Buffer26 Size is global var used in other routines that need to know size
  for(uint32_t i=0;i<PassedBuffer26Size;i++)    // Set all bytes in buffer to 0x7f (DAC silence), TEB Sep-23-2019
    Buffer26[i]=0x7f;
  FirstPlayListItem26=nullptr;                      // TEB, Oct-10-2019
  DacPin26=TheDacPin26;               // set dac pin to use
  LastDacValue26=0x7F;                // set to mid point
  dacWrite(DacPin26,LastDacValue26);          // Set speaker to mid point, stops click at start of first sound
  InitSineValues26();               // create our table of sine values for our special circular
                          // angles of 0 - 255 "DAC Degrees". Speeds things up to if
                          // you pre-calculate
  // Set up interrupt routine
  timer26 = timerBegin(TimerNo, 80, true);          // use timer26 TimerNo, pre-scaler is 80 (divide by 8000), count up
  timerAttachInterrupt(timer26, &onTimer26, true);    // P3= edge triggered
  timerAlarmWrite(timer26, 20, true);               // will trigger 250,000 times per second,
  timerAlarmEnable(timer26);                        // enable
  delay(1);                                   // Allow system to settle, otherwise garbage can play for first second

}


void XT_DAC_Audio26_Class::PrintPlayList()
{
  // For debugging purposes, lists addresses of all items in playlist
    Serial.println("Current play list");
  XT_PlayListItem26_Class *PlayItem = nullptr;  // TEB, Oct-10-2019
  PlayItem=FirstPlayListItem26;
...
void XT_DAC_Audio26_Class::RemoveFromPlayList(XT_PlayListItem26_Class *ItemToRemove)
{
...
void XT_DAC_Audio26_Class::Play(XT_PlayListItem26_Class *Sound)
{
  Play(Sound,true);       // Sounds mix by default
}


void XT_DAC_Audio26_Class::Play(XT_PlayListItem26_Class *Sound,bool Mix)
{
  ...
  if(FirstPlayListItem26==nullptr) // no items to play in list yet. TEB, Oct-10-2019
  {
    FirstPlayListItem26=Sound;
    LastPlayListItem26=Sound;
  }
  else{
    // add to end of list
    LastPlayListItem26->NextItem=Sound;
    Sound->PreviousItem=LastPlayListItem26;
    LastPlayListItem26=Sound;
...
bool XT_DAC_Audio26_Class::AlreadyPlaying(XT_PlayListItem26_Class *Item)
{
  // returns true if sound already in list of items to play else false
  XT_PlayListItem26_Class* PlayItem;
  PlayItem=FirstPlayListItem26;
  while(PlayItem!=nullptr)            // TEB, Oct-10-2019
  {
    if(PlayItem==Item)
...
void XT_DAC_Audio26_Class::StopAllSounds()
{
...
XT_Wav26_Class::XT_Wav26_Class(const unsigned char *WavData)
{
...
uint8_t XT_Wav26_Class::NextByte()
{
...
//  ReturnValue=Data[DataIdx];        // by default we return previous value;
    ReturnValue = SetVolume26(Data[DataIdx], Volume);  // TEB, Oct-22-2019
...
XT_Instrument26_Class::XT_Instrument26_Class():XT_Instrument26_Class(INSTRUMENT_PIANO,127)
{
...
XT_Instrument26_Class::XT_Instrument26_Class(int16_t InstrumentID):XT_Instrument26_Class(InstrumentID,127)
{
    // See main constructor routine for description of passed parameters
    // and defaults
}
...
XT_Instrument26_Class::XT_Instrument26_Class(int16_t InstrumentID, uint8_t Volume)
{
...
void XT_Instrument26_Class::SetNote(int8_t Note)
{
  this->Note=abs(Note);
}


void XT_Instrument26_Class::SetInstrument(uint16_t Instrument)
{
...
void XT_Instrument26_Class::SetDefaultInstrument()
{
  SetWaveForm(WAVE_SQUARE);
}


void XT_Instrument26_Class::SetPianoInstrument()
{
  SetWaveForm(WAVE_TRIANGLE);
  FirstEnvelope=new XT_Envelope26_Class();
...
void XT_Instrument26_Class::SetHarpsichordInstrument()
{
...
void XT_Instrument26_Class::SetOrganInstrument()
{
...
void XT_Instrument26_Class::SetSaxophoneInstrument()
{
  SetWaveForm(WAVE_SQUARE);
  FirstEnvelope=new XT_Envelope26_Class();
  FirstEnvelope->AddPart(15,127);
...
void XT_Instrument26_Class::SetWaveForm(uint8_t WaveFormType)
{
  ...
  switch (WaveFormType)
  {
    case WAVE_SQUARE : WaveForm=new XT_SquareWave26_Class();break;
    case WAVE_TRIANGLE : WaveForm=new XT_TriangleWave26_Class();break;
    case WAVE_SAWTOOTH : WaveForm=new XT_SawToothWave26_Class();break;
    case WAVE_SINE : WaveForm=new XT_SineWave26_Class();break;

    default: // compilation error, default to square
      WaveForm=new XT_SquareWave26_Class();break;
  }
}

void XT_Instrument26_Class::SetFrequency(uint16_t Freq)
{
...
void XT_Instrument26_Class::Init()
{
  CurrentByte=0;
  WaveForm->Init(Note);
...
uint8_t XT_Instrument26_Class::NextByte()
{
  uint8_t ByteToPlay;
  ...
  return SetVolume26(ByteToPlay,Volume);
}


XT_Envelope26_Class *XT_Instrument26_Class::AddEnvelope()
{
  ...
  XT_Envelope26_Class* Env=new XT_Envelope26_Class();
  XT_Envelope26_Class* ThisEnv,PreviousEnv;
...
void XT_Instrument26_Class::SetDuration(uint32_t Length)      // in millis
{
...
XT_MusicScore26_Class::XT_MusicScore26_Class(int8_t* Score):XT_MusicScore26_Class(Score,TEMPO_ALLEGRO,&DEFAULT_INSTRUMENT26)
{
  // set tempo and instrument to default, number of plays to 1 (once)
}


XT_MusicScore26_Class::XT_MusicScore26_Class(int8_t* Score,uint16_t Tempo):XT_MusicScore26_Class(Score,Tempo,&DEFAULT_INSTRUMENT26)
{
  // set instrument to default and plays to 1
}


XT_MusicScore26_Class::XT_MusicScore26_Class(int8_t* Score,uint16_t Tempo,XT_Instrument26_Class* Instrument)
{
  // Create music score
  this->Score=Score;
  this->Tempo=Tempo;
...
XT_MusicScore26_Class::XT_MusicScore26_Class(int8_t* Score,uint16_t Tempo,uint16_t InstrumentID)
{
  // Create music score
  this->Score=Score;
  this->Tempo=Tempo;
  this->Instrument=&DEFAULT_INSTRUMENT26;
  Instrument->SetInstrument(InstrumentID);
}


void XT_MusicScore26_Class::Init()
{
...
uint8_t XT_MusicScore26_Class::NextByte()
{
...
void XT_MusicScore26_Class::SetInstrument(uint16_t InstrumentID)
{
  Instrument->SetInstrument(InstrumentID);
}
...
XT_Envelope26_Class::XT_Envelope26_Class()
{
  // nothing yet
}
...
XT_Envelope26_Class::~XT_Envelope26_Class()
{
  ...
  XT_EnvelopePart26_Class* Part,*NextPart;
...
uint8_t XT_Envelope26_Class::NextByte(uint8_t ByteToPlay)
{
  ...
  if(FirstPart!=0)
  {
    ...
    if(EnvelopeCompleted==false)
    {
      ...
      if(CurrentEnvelopePart!=0)
      {
        ...
        ByteToPlay=SetVolume26(ByteToPlay,int(CurrentVolume));
        DurationCounter--;
...
void XT_Envelope26_Class::InitEnvelopePart(XT_EnvelopePart26_Class* EPart,uint8_t LastVolume)
{
...
XT_EnvelopePart26_Class* XT_Envelope26_Class::AddPart(uint16_t Duration,uint16_t StartVolume,uint16_t TargetVolume)
{
  XT_EnvelopePart26_Class* EPart;
...
XT_EnvelopePart26_Class* XT_Envelope26_Class::AddPart(uint16_t Duration,uint16_t TargetVolume)
{
  // creates and adds an envelope part to this current envelope
  XT_EnvelopePart26_Class* EPart=new XT_EnvelopePart26_Class();
...
// Envelope part class
void XT_EnvelopePart26_Class::SetDuration(uint16_t Duration)
{
  this->Duration=Duration;
  this->RawDuration=50*Duration;
}

uint16_t XT_EnvelopePart26_Class::GetDuration()
{
  return Duration;
}
...
uint8_t XT_SquareWave26_Class::NextByte()
{
...
void XT_SquareWave26_Class::Init(int8_t Note)
{
  if(Note!=-1)                // use the note not a raw frequency
    Frequency=XT_Notes26[Note];         // if -1 then use the raw frequency
  if(Frequency>25000)
    Frequency=25000;
...
uint8_t XT_TriangleWave26_Class::NextByte()
{
  // returns the next byte for this frequency of a square wave
...
void XT_TriangleWave26_Class::Init(int8_t Note)
{
  NextAmplitude=127;
  if(Note!=-1)                // use the note not a raw frequency
    Frequency=XT_Notes26[Note];         // if -1 then use the raw frequency
...
uint8_t XT_SawToothWave26_Class::NextByte()
{
  // returns the next byte for this frequency of a square wave
...
void XT_SawToothWave26_Class::Init(int8_t Note)
{
  NextAmplitude=0;
  if(Note!=-1)                // use the note not a raw frequency
    Frequency=XT_Notes26[Note];         // if -1 then use the raw frequency
...
///////////////////////////////////////////////
uint8_t XT_SineWave26_Class::NextByte()
{
  // returns the next byte for this frequency of a sine wave
  CurrentAngle+=AngleIncrement;
  if(CurrentAngle>255)
    CurrentAngle=0;             // Set to start
  return SineValues26[int(CurrentAngle)];
}

void XT_SineWave26_Class::Init(int8_t Note)
{
  CurrentAngle=0;
  if(Note!=-1)                // use the note not a raw frequency
    Frequency=XT_Notes26[Note];         // if -1 then use the raw frequency
...
void XT_Sequence26_Class::AddPlayItem(XT_PlayListItem26_Class *PlayItem)
{
  ...
  XT_SequenceItem26_Class *SequenceItem;
  // create a new sequence item
  SequenceItem=new XT_SequenceItem26_Class();
...
void XT_Sequence26_Class::RemoveAllPlayItems()
{
  // Remove all items in linked list and free associated memory

  XT_SequenceItem26_Class *ThisItem,*NextItem;
...
uint8_t XT_Sequence26_Class::NextByte()
{
  if(CurrentItem == nullptr )     // TEB, Oct-10-2019.
    return 0;
  if(CurrentItem->PlayItem->Playing)
  //  return CurrentItem->PlayItem->NextByte();
        return SetVolume26(CurrentItem->PlayItem->NextByte(), Volume);  // TEB, Oct-22-2019
...
XT_FilterNoise26_Class::XT_FilterNoise26_Class(int8_t AmountOfNoise)
{
...
XT_FilterNoise26_Class::XT_FilterNoise26_Class(int8_t Min,int8_t Max)
{
  this->Min=Min;
  this->Max=Max;
}
...
uint8_t XT_FilterNoise26_Class::FilterWave(uint8_t TheByte)
{
