...
//   Added DacVolume feature to XT_DAC_Audio26_Class
//   Revised pointer declarations to use cpp nullptr instead of NULL/zero
//   Set XT_PlayListItem26_Class's default Volume to maximum (127).
//
...
#define BUFFER_SIZE_DEFAULT 4000 			    // Size of buffer to store data to send to DAC. 3 bytes minimum!
												...
												// Buffer26 memory. Which can be done in your code when you create the
												// DacAudio object
												// If your main loop is very very slow but you want to keep buffer
												// memory low then consider calling the DAC_Audio_Class FillBuffer26
												...
												// Use the routine MaxBuffer26Usage in your main loop to output the max
...
class XT_Envelope26_Class;						// forward ref for this class


class XT_Filter26_Class
{
...
class XT_FilterNoise26_Class : public XT_Filter26_Class
{
	...
	XT_FilterNoise26_Class(int8_t Min,int8_t Max);
	XT_FilterNoise26_Class(int8_t AmountOfNoise);

	uint8_t FilterWave(uint8_t TheByte);

};

class XT_PlayListItem26_Class
{
	private:
	...
	XT_PlayListItem26_Class *PreviousItem = nullptr; // TEB, Sep-30-2019
	XT_PlayListItem26_Class *NextItem = nullptr;     // TEB, Sep-30-2019
	XT_Filter26_Class *Filter = nullptr;             // TEB, Oct-10-2019
	...
	XT_PlayListItem26_Class();
};



// The Main Wave class for sound samples
class XT_Wav26_Class : public XT_PlayListItem26_Class
{
	private:
	protected:
	...
	// constructors
	XT_Wav26_Class(const unsigned char *WavData);
...
class XT_EnvelopePart26_Class
{
	...
	XT_EnvelopePart26_Class *PreviousPart=nullptr;        // TEB, Oct-10-2019
	XT_EnvelopePart26_Class *NextPart=nullptr;            // TEB, Oct-10-2019
...
class XT_Envelope26_Class
{
	...
	public:
	XT_EnvelopePart26_Class *FirstPart=nullptr;          	// start of linked list of envelope parts. TEB, Oct-10-2019
	XT_EnvelopePart26_Class *LastPart=nullptr;          	// start of linked list of envelope parts. TEB, Oct-10-2019
	XT_EnvelopePart26_Class *CurrentEnvelopePart=nullptr;	// Current active envelope part. TEB, Oct-10-2019

	XT_Envelope26_Class *NextEnvelope=nullptr;		// Next envelope of any, TEB, Oct-10-2019
	...
	~XT_Envelope26_Class();
	void Init();
	XT_EnvelopePart26_Class *AddPart(uint16_t Duration,uint16_t TargetVolume);  // 0 to Max 127
	XT_EnvelopePart26_Class *AddPart(uint16_t Duration,uint16_t StartVolume,uint16_t TargetVolume);  // 0 to Max 127
	void InitEnvelopePart(XT_EnvelopePart26_Class* EPart,uint8_t LastVolume);
...
class XT_SquareWave26_Class : public XT_Wave26_Class
{
	// Square wave class
	public:
	uint8_t NextByte();
...
class XT_TriangleWave26_Class : public XT_Wave26_Class
{
	// Square wave class
	public:
	uint8_t NextByte();
...
class XT_SawToothWave26_Class : public XT_Wave26_Class
{
	// Square wave class
	public:
	uint8_t NextByte();
...
class XT_SineWave26_Class : public XT_Wave26_Class
{
	// Square wave class
	private:
...
class XT_Instrument26_Class : public XT_PlayListItem26_Class
{
	...
	public:
		...
		XT_Envelope26_Class *FirstEnvelope=nullptr;   // First envelope associated with this Instrument. TEB, Oct-10-2019.
		XT_Envelope26_Class *CurrentEnvelope=nullptr;	// Current envelope in force. TEB, Oct-10-2019.
		XT_Wave26_Class *WaveForm=nullptr;		    // i.e. square, sawtooth, sine, triangle. TEB, Oct-10-2019.

		// Constructors
		XT_Instrument26_Class();
		XT_Instrument26_Class(int16_t InstrumentID);
		XT_Instrument26_Class(int16_t InstrumentID, uint8_t Volume);
		...
		XT_Envelope26_Class *AddEnvelope();
...
class XT_MusicScore26_Class:public XT_PlayListItem26_Class
{
	...
	public:
		XT_Instrument26_Class *Instrument = nullptr;		    // The instrument to use. TEB, Oct-10-2019
		...
		// constructors
		XT_MusicScore26_Class(int8_t* Score);
		XT_MusicScore26_Class(int8_t* Score,uint16_t Tempo);
		XT_MusicScore26_Class(int8_t* Score,uint16_t Tempo,XT_Instrument26_Class* Instrument);
		XT_MusicScore26_Class(int8_t* Score,uint16_t Tempo,uint16_t InstrumentID);
...
class XT_SequenceItem26_Class
{
	// An item that contains information about what to play and links to the next item
	// in the list of things to play
	public:
	XT_PlayListItem26_Class *PlayItem=nullptr;		// The item to play.  TEB, Oct-10-2019
	XT_SequenceItem26_Class *NextItem=nullptr;		// Next item to play. TEB, Oct-02-2019.
};


class XT_Sequence26_Class:public XT_PlayListItem26_Class
{
	// allows you to queue up many playlist items one after the other with minimum effort

	private:
		XT_SequenceItem26_Class *CurrentItem=nullptr;			// current item playing from within the list. TEB, Oct-02-2019.
		XT_SequenceItem26_Class *FirstItem=nullptr;          	// first play list item to play in linked list. TEB, Oct-02-2019.
		XT_SequenceItem26_Class *LastItem=nullptr;          	// last play list item to play in linked list. TEB, OCt-02-2019.
	...
	public:
		...
		uint8_t NextByte();
		void Init();
		void AddPlayItem(XT_PlayListItem26_Class *PlayItem);
...
class XT_DAC_Audio26_Class
{

	public:

		XT_DAC_Audio26_Class(uint8_t TheDacPin, uint8_t TimerNo);
		XT_DAC_Audio26_Class(uint8_t TheDacPin, uint8_t TimerNo,uint16_t PassedBuffer26Size);

		void FillBuffer26();										// Fills the buffer of values to send to the DAC, put in your
		...
		void Play(XT_PlayListItem26_Class *Sound);
		void Play(XT_PlayListItem26_Class *Sound,bool Mix);
		void StopAllSounds();
		bool AlreadyPlaying(XT_PlayListItem26_Class *Item);
		void RemoveFromPlayList(XT_PlayListItem26_Class *ItemToRemove);
		int Buffer26Usage();
